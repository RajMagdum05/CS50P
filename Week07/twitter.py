...
###* (?: ) is used to tell interpretor not to capture this group

if matches := re .search(r"^(?:https?://)?(?:www\.)?twitter\.com\([a-z0-9_])$", url, re.IGNORECASE):
    username = matches.group(1)                                  # it is in () only bcz to capture it

    # so now here we put 1 in bracket bcz first two brackets are not captured
